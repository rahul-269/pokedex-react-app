...
const Popup = ({ pokemon, onClose }) => {
  ...
  const bothType = (type) => {
    if (length === 2) {
      ...
      return (
        <>
          <span className={`popup-type ${type[0].type.name}`}>{type1}</span>
          <span className={`popup-type ${type[1].type.name}`}>{type2}</span>
        </>
      );
    } else {
      const type1 = type[0].type.name.toUpperCase();
  ...
  const ucaseFirst = (pkname) => {
    return pkname.charAt(0).toUpperCase() + pkname.slice(1).toLowerCase();
  };
      ...
      <div className="popup-content">
        <div className="popup-header">
          <div className="popup-image">
            <motion.img
              className="popup-image-sprite"
              key={id}
              src={image}
              alt={name}
              animate={{ rotate: [0, -15, 15, 0], y: [0, 5, -15, 0] }}
              transition={{ delay: 1 }}
            />
          </div>
        ...
        <div className="popup-body">
          <div>
            ...
            {selected === 1 && (
              <div className="popup-body-info">{allAbilities(abilities)}</div>
            )}

            {selected === 2 && (
              <div className="popup-moves-info">
                <div className="moves-header">Learnable Moves</div>
                {moves.map((move) => (
                  <div key={move.move.name} className="popup-moves-item">
                    <span>{ucaseFirst(move.move.name)}</span>
                  </div>
                ))}
              </div>
            )}
            ...
            {selected === 3 && (
              <div className="popup-shiny-info">
                <div className="popup-image">
                  <motion.img
                    className="popup-image-sprite"
                    key={id}
          ...
          <div className="dpad-buttons">
            <div className="toggle-buttons">
              <div className="button-group">
                <div className="stats-button" onClick={() => setSelected(0)}>
                  {" "}
                  STATS{" "}
                </div>
                <div className="moves-button" onClick={() => setSelected(2)}>
                  {" "}
                  MOVES{" "}
                </div>
              </div>
              <div className="button-group">
                ...
                <div className="shiny-button" onClick={() => setSelected(3)}>
                  {" "}
                  SHINY{" "}
                </div>
              </div>
